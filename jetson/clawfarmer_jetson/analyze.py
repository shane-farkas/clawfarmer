...
def rich_analyze(
    ...
    """Send an image + arbitrary prompt to Ollama and return the generated text.

    Same transport as analyze_image but the prompt is caller-supplied — used
    by host-tick to pass a prompt containing sensor context + AGENTS.md
    thresholds so Gemma can cross-reference the image with sensors.
    """
    p = Path(path)
    if not p.exists():
        return {"ok": False, "error": f"image not found: {path}", "at": _now_iso()}
